...
export const dictPrim = (vals: List<Value>): EmptySExp | CompoundSExp => {
    vals.forEach((v: Value) => isPairPrim(v) ? v : makeFailure(`Dict: param is not compound ${format(v)}`));
    return isNonEmptyList<Value>(vals) ? makeCompoundSExp(first(vals), listPrim(rest(vals))) :
    makeEmptySExp();
}